...
Four options in `agent.ts` are fixed for every call and must not be loosened without reading [ADR-008](docs/adr/008-claude-code-subscription.md):

- `tools: []` — **the security boundary.** Obserf feeds untrusted text straight into the prompt, so the model must have no tools at all. `allowedTools` is the auto-approval list, not the availability list; it is not a substitute.
