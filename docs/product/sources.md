...
**Hacker News search can include killed submissions and their comments.** The adapter drops hits whose `title` (or `story_title` when `title` is null or absent) is exactly `[dead]`, before they reach the gate or assessment. Comment text can otherwise pass the `thin` gate even though [HN closes killed submissions to new comments](https://news.ycombinator.com/newsfaq.html). This checks the index marker; it does not establish that every surviving thread is open to replies.
...
**Brave derives venue keys from URLs** so `venueGuidance`, which matches exactly, can also apply to its results: Reddit `/r/<subreddit>` paths use `r/<subreddit>`, and GitHub paths with at least two non-empty segments use `github.com/<first>/<second>`. Repository subpages therefore share their repository's key. This is path-based naming, not repository validation: `github.com/topics/rust` also keeps both segments. Other URLs use the normalized host.
